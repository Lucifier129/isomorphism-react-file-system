...
class File extends Document {
    constructor(path) {
        ...
        this.lastModifyTime = [mtime.toLocaleDateString(), mtime.toLocaleTimeString()].join(' ')
    }
    get extname() {
...
class Directory extends Document {
    ...
    removeChild(name) {
        return this.proxyPromise('removeChild' + name, () => {
            return this.getChild(name)
            .then((child) => child && child.remove())
        })
    }
    removeProgeny(path) {
        return this.proxyPromise('removeProgeny' + path, () => {
            return this.getProgeny(path)
            .then((progeny) => progeny && progeny.remove())
        })
    }
    ...
    toRelativeJson(path) {
        let sep = JSON.stringify(path || this.path).replace(/["']/g, '')
        let result = this.json().split(sep).join('')
        return result
    }
}
